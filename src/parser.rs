...
impl Parser {
    ...
    pub fn convert_data(&mut self, path: &str) -> Result<(), Error> {
            ...
            match tag.as_str() {
                text => {
                    let mut table: Box<dyn Table> = match text {
                        ...
                        "strength_limits_sea" => Box::new(StrengthForceLimit::new(
                            "sea\r\n".to_owned() + &body,
                            Rc::clone(&self.physical_frame.clone().ok_or(Error::FromString(
                                format!("Parser LoadConstant error: physical_frame"),
                            ))?),
                        )),
                        "strength_limits_harbor" => Box::new(StrengthForceLimit::new(
                            "harbor\r\n".to_owned() + &body,
                            Rc::clone(&self.physical_frame.clone().ok_or(Error::FromString(
                                format!("Parser LoadConstant error: physical_frame"),
                            ))?),
        ...
        let mut compartment_curve_data: Xlsx<_> =
            open_workbook(path.to_owned() + "Tank_curve.xlsx").expect("Cannot open file");
        let compartment_curve_data: Vec<(String, Range<Data>)> = compartment_curve_data
            .worksheets()
            .into_iter()
        ...
        let mut hold_curve_data: Xlsx<_> =
            open_workbook(path.to_owned() + "Holds_curve.xlsx").expect("Cannot open file");
        let hold_curve_data: Vec<(String, Range<Data>)> = hold_curve_data
            .worksheets()
            .into_iter()
            .filter(|(_, range)| range.used_cells().count() > 0)
            .collect();
    ...
    pub fn convert_tests(&mut self, path: &str) -> Result<(), Error> {
        ...
        let workbook: Vec<(String, Range<Data>)> = workbook
            .worksheets()
            .into_iter()
            .filter(|(_, range)| range.used_cells().count() > 0)
            .collect();
    ...
    pub fn write_to_file(self) -> Result<(), Error> {
        ...
        if let Some(general) = self.general {
            ...
            std::fs::write(format!("../{ship_name}/test/data.sql"), tests)
