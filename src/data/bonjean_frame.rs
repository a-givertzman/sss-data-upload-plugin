...
impl Table for BonjeanFrame {
    ...
    fn parse(&mut self) -> Result<(), Error> {
     //   dbg!(&self.data);
        let data = self.data.take().ok_or(Error::FromString(
            "BonjeanFrame data error: no data!".to_owned(),
        ))?;
        let data = data.replace(",", ".");
        let mut data: Vec<Vec<&str>> = data
            .split("\r\n")
            .filter_map(|line| {
                if line.len() > 2 {
                    Some(line.split(';').filter(|s| s.len() > 0).collect())
                } else {
                    None
                }
            })
  ...
  //          dbg!(&row);
    ...
    fn to_sql(&mut self, id: usize) -> Vec<String> {
        ...
        self.pos_x.iter().enumerate().for_each(|(frame_index, x)| {
